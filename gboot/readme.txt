...
available. The tiny build fits in 1k (2 flash pages) the stardard build uses 1.5k (3 flash pages).
The tiny build is currently only available for the "basic" chip variants (F320, F342 and F343). 
The builds for the other variants are currently a few bytes too long to fit in 1k.
...
sdcc (4.2 or 3.3 recommended, other versions should work, 3.0 has issues with inline functions)
GNU make & sercord (on linux) or Silicon Laboratories IDE (on windows)

Configuring gboot
----------------------------
The distribution defaults to 16k flash and one UART (F320) with the 
ID set to 0xFFFF, version to 0x10 and PCB to 0xA0. There are a few simple 
...
----------------------------------------
All is required is to add --code-loc=0x0200 (tiny build) or --code-loc 0x0400
(standard build) to the link stage of your firmware. Convert the intel hex 
file to bin using srec_cat, hex2bin or your favorite tool (make sure the the
code start at 0 in the bin file) and use the included flash utility to upload
the code.
...
is corrupted (location 0x0200/0x0400 does not contain 0x02).
...
Flash Layout
--------------
The tiny build the bootloader occupies the first page (0x0000-0x01ff),
the standard build occupies the first two pages (0x0000-0x03ff).
Both also use the last page of flash (0x3c00-0x3dfe on a 16k device).
